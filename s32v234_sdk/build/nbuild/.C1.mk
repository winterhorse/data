...
%_APU_LOAD.h: %_APU_LOAD.elf
	$(eval BASENAME=$(subst _APU_LOAD.elf,,$<))
	$(call nbmsg,"$(C1_PROMPT) READELF" $@) \
	$(READELF) -pPMh=32 -mDMb=8 -mVMb=1024 $< -eGb -t $(TCL_SCRIPT) -Tload_name=$(BASENAME) > $@
	@# Patching READELF output to include support for APU stdio, because this functionality doesn't exist in $(READELF)
	@echo '#ifndef _$(BASENAME)_STDIO_APU_LOAD_H_' >> $@
	@echo '#define _$(BASENAME)_STDIO_APU_LOAD_H_' >> $@
	$(eval APEX_LOG_BUFFER=$(shell grep APEX_LOG_BUFFER $<.map | cut -d'.' -f 1 | tr -d ' '))
	@if [ -n "$(APEX_LOG_BUFFER)" ]; then \
		echo "const uint32_t $(BASENAME)_APEX_LOG_BUFFER = $(APEX_LOG_BUFFER);" >> $@; \
	else \
		echo "const uint32_t $(BASENAME)_APEX_LOG_BUFFER = 0;" >> $@; \
	fi
	$(eval APEX_LOG_SIZE=$(shell grep APEX_LOG_SIZE $<.map | cut -d'.' -f 1 | tr -d ' '))
	@if [ -n "$(APEX_LOG_SIZE)" ]; then \
		echo "const uint32_t $(BASENAME)_APEX_LOG_SIZE = $(APEX_LOG_SIZE);" >> $@; \
	else \
		echo "const uint32_t $(BASENAME)_APEX_LOG_SIZE = 0;" >> $@; \
	fi
	@echo '#endif' >> $@
