...
{
   mkernel_list_handle = 0;
   mkernel_list = 0;
   mkernel_num  = 0;
}
KernelManager::~KernelManager()
{
   if (mkernel_list_handle != 0)
   {
      OAL_MemoryFree(mkernel_list_handle);
      mkernel_list_handle = 0;
      mkernel_list = 0;
      mkernel_num  = 0;
   }
...
int KernelManager::InitLoadKernelDB( int /*apuid*/, const LOAD_SEGMENT_t* seg_addr)
{
   ...
   while (memory_type != -1)
   {
      ...
      if ((dst_offset == DMEM_KERNEL_DB_OFFSET) && (memory_type == APU_LOAD_ID_DMEM))
      {
         ...
         {
            mkernel_num  = kn;
            mkernel_list_handle = OAL_MemoryAllocFlag(
                                       (sizeof(KERNEL_INFO)* kn),
                                       OAL_MEMORY_FLAG_ALIGN(ALIGN2_CACHELINE) |
                                       OAL_MEMORY_FLAG_CONTIGUOUS |
                                       OAL_MEMORY_FLAG_ZERO);
            mkernel_list = (KERNEL_INFO*)OAL_MemoryReturnAddress(mkernel_list_handle, ACCESS_CH_WB);
