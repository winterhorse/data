...
static void RGB888X_TO_Y_nativekernel(void* pArgs)
{
   ...
   R2Y.Init2D(0, (void*)lpIo->pBuf, lpIo->nd_size.width, lpIo->nd_size.height, lpIo->nd_span.height, lpIo->elementDataType, lpIo->elementDimX, lpIo->elementDimY);

   //G2Y
   ACF_DataDesc G2Y;
   lpIo = (acf_io_info*)((int32_t)pArgs+2*sizeof(acf_io_info));
   G2Y.Init2D(0, (void*)lpIo->pBuf, lpIo->nd_size.width, lpIo->nd_size.height, lpIo->nd_span.height, lpIo->elementDataType, lpIo->elementDimX, lpIo->elementDimY);

   //B2Y
   ACF_DataDesc B2Y;
   lpIo = (acf_io_info*)((int32_t)pArgs+3*sizeof(acf_io_info));
   B2Y.Init2D(0, (void*)lpIo->pBuf, lpIo->nd_size.width, lpIo->nd_size.height, lpIo->nd_span.height, lpIo->elementDataType, lpIo->elementDimX, lpIo->elementDimY);
